...
if username_given == real_username and password_given == real_password:
    ...
    while True:
        ...
        if response == '1':
            ...
            if response_for_journal == '1':
            ...
            elif response_for_journal == '2':
                os.chdir(cwd)
                os.chdir('journals')
                dates = os.listdir()
                dict_date = {}
                i = 1
                print("Enter the index of the date of your journal.")
                for date in dates:
                    date_in_loop = list(date)
                    counter = -1
                    real_date = ''

                    for element in date_in_loop:
                        dict_date.update({i: date})
                        real_date += element
                ...
                response = input(">>>")
                os.chdir(dict_date[int(response)])

                print("Enter the index of time of your journal")
                times_in_folder = os.listdir()
                dict_time = {}
                j = 1
                for times in times_in_folder:
                    time_in_loop = list(times)
                    real_time = ''
                    counter = -1

                    for element in time_in_loop:
                        dict_time.update({j: times})
                        real_time += element
                        if counter % 2 == 0 and counter <= 2:
                            real_time += ":"
                        counter += 1
                    time_to_be_displayed = real_time.split(":")
                    if int(time_to_be_displayed[1]) > 12:
                        time_to_be_displayed[1] = int(time_to_be_displayed[1]) - 12
                        time_to_be_displayed.append("PM")
                        actual_time = str(time_to_be_displayed[1]) + ":" + time_to_be_displayed[0] + \
                                      time_to_be_displayed[-1]
                    else:
                        time_to_be_displayed.append("AM")
                        actual_time = str(time_to_be_displayed[1]) + ":" + time_to_be_displayed[0] + " " + \
                                      time_to_be_displayed[-1]
                    print(j, " -- ", actual_time)
                    j += 1
                ...
                response = input(">>>")
                subprocess.Popen(["notepad.exe", dict_time[int(response)]])
                os.chdir(cwd)
            ...
            print("You can add your own songs by editing the songs.txt in the directory or remove those you don't want")
            time.sleep(2)
            clear()
            os.chdir(cwd)
